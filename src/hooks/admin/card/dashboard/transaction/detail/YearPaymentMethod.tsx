...
export default function useYearPaymentMethodTransactionDetail(toast: any) {
    const {
    ...
    const fetchYearlyPaymentMethod = useCallback(
        async (year: number, card_number: string) => {
            try {
                ...
                await findYearTransactionMethodCard(toast, year, card_number);
            } catch (error) {
        ...
        [findYearTransactionMethodCard, setLoadingYearTransactionMethod, setErrorYearTransactionMethod],
    );
